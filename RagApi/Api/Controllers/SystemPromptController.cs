...
namespace RagApi.Api.Controllers
{
    ...
    public class SystemPromptController : BaseController
    {
        ...
        private readonly SystemPromptMapper _systemPromptMapper;

        public SystemPromptController(
            IRequestContext requestContext,
            ISystemPromptService systemPromptService,
            SystemPromptMapper systemPromptMapper)
            : base(requestContext) // Poistettu mapper
        {
            _systemPromptService = systemPromptService;
            _systemPromptMapper = systemPromptMapper;
        }
        ...
        public async Task<IActionResult> GetSystemPrompts()
        {
            try
            {
                ...
                var systemPromptDtos = systemPrompts.Select(sp => _systemPromptMapper.MapToDto(sp)).ToList();
                return Success(systemPromptDtos);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> GetSystemPrompt(string id)
        {
            try
            {
                ...
                var systemPromptDto = _systemPromptMapper.MapToDto(systemPrompt);
                return Success(systemPromptDto);
            }
            catch (Exception ex)
            {
        ...
        public async Task<IActionResult> CreateSystemPrompt([FromBody] CreateSystemPromptRequest request)
        {
            ...
            try
            {
                // Käytä mapper-luokkaa muuntamaan request entity-objektiksi
                var entity = _systemPromptMapper.MapToEntity(request);
                ...
                var systemPromptDto = _systemPromptMapper.MapToDto(systemPrompt);
                return Created(systemPromptDto, nameof(GetSystemPrompt), new { id = systemPrompt.Id });
            }
            catch (Exception ex)
            {
